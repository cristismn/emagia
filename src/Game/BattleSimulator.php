...
class BattleSimulator
{
    ...
    private function simulate()
    {
        ...
        while ($this->defender->getStat('hp') >= 0 && $turns <= $this->totalTurns) {
            echo '# Turn ' . $turns . ' - ' . $this->attacker->name . ' attacks! #' . PHP_EOL;
            ...
            if ($dodgeAttack) {
                echo 'dodge the hit from ' . $this->attacker->name . '.' . PHP_EOL;
            }
            ...
            if (!$dodgeAttack) {
                if ($this->defender->hasSpecialSkills()) :
                    ...
                    if ($defendSkill != false) :
                        echo '***' . $defendSkill['name'] . '***' . PHP_EOL;
                        switch ($defendSkill['key']) {
                            case "magic_shield":
                                $this->damage = $this->damage / 2;
                                break;
                            case "first_aid":
                                echo 'Sorry, our Hero don`t have bandages.' . PHP_EOL;
                                break;
                        }
                    endif;
                endif;
        ...
        echo $this->checkWinner()->name . ' wins!' . PHP_EOL;
    }
